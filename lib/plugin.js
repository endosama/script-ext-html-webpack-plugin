...
class ScriptExtHtmlWebpackPlugin {
  ...
  alterAssetTagsCallback(compilation, pluginArgs, callback) {
    ...
    const fullFileName = path.resolve(options.compilationOptions.output.path, pluginArgs.outputName);
    try {
      ...
      if (elements.shouldUpdate(options)) {
        ...
        pluginArgs[headTagName] = elements.update(compilation.assets, options, pluginArgs[headTagName], fullFileName);
        debugEvent('replacing <body> <script> elements');
        pluginArgs[bodyTagName] = elements.update(compilation.assets, options, pluginArgs[bodyTagName], fullFileName);
      }
